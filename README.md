...
TrickTrace 是一个在浏览器中运行的桥牌分析工具，提供图片识牌、DDS 双明手求解、逐张回放和最优牌路追踪。图片识别与求解在本地浏览器执行，模型、WASM 和字体由站点提供，无需配置云端 AI API。
...
- **条件模拟**：两家牌与单明手首攻分析，按点力、花色长度和高级条件采样；CCBA v2.2 叫牌规则可联动部分手牌约束。
