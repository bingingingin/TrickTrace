...
当前训练及验证图片生成脚本依赖 Windows 系统字体；其他系统需要调整字体路径并准备可用字体。训练读取 Windows 字体和 `.tools/fonts` 中的可选字体；具体字体清单写入模型 JSON。缺少字体会改变训练数据，不能声称跨机器逐字节复现。随机种子为 418。新模型先写入 `.cache/models-v4`，通过整图对照后才复制到 `public/models`。合成字符准确率不能替代整图准确率；原始用户截图不参加训练。
