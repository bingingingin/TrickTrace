...
labels=list('23456789TJQKA')+['10','S','H','D','C']
glyphs=list('23456789TJQKA')+['10','♠','♥','♦','♣']
...
fonts=[p for n in names if (p:=fontroot/n).exists()]
def render(index,angle=None,font=None):
 f=ImageFont.truetype(str(font or random.choice(fonts)),random.randint(30,70));im=Image.new('L',(160,160));d=ImageDraw.Draw(im);d.text((35,20),glyphs[index],font=f,fill=255,stroke_width=random.choice([0,0,0,1]));box=im.getbbox();im=im.crop(box)
 im=im.resize((max(2,int(im.width*random.uniform(.7,1.3))),im.height))
 im=im.rotate(random.uniform(-55,55) if angle is None else angle,expand=True,resample=Image.Resampling.BILINEAR);box=im.getbbox();im=im.crop(box)
...
if __name__=='__main__':
 ...
 cache=root/'.cache/glyphs.npz'
 if cache.exists():
  a=np.load(cache);x=torch.tensor(a['x'],dtype=torch.float32);y=torch.tensor(a['y'])
 else:
  print('Generating 21600 synthetic glyphs',flush=True)
  x=[];y=[]
  for i in range(len(labels)):
   for j in range(1200):x.append(render(i));y.append(i)
 ...
 order=torch.randperm(len(y));train=order[:20000];val=order[20000:]
 ...
 net.cpu().eval();torch.onnx.export(net,torch.zeros(1,1,32,32),str(out/'glyphs.onnx'),input_names=['image'],output_names=['logits'],dynamic_axes={'image':{0:'batch'},'logits':{0:'batch'}},opset_version=17)
 (out/'glyphs.json').write_text(json.dumps({'labels':labels,'size':32,'seed':418,'training':'synthetic system fonts only; no supplied screenshots','train':len(train),'validation':len(val),'validation_accuracy':acc,'fonts':[p.name for p in fonts]},indent=2),encoding='utf8')
