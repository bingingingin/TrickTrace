...
export interface Board {id:string;name:string;number?:number;singleDummySourceId?:string;dealer:Seat;vulnerability:'None'|'NS'|'EW'|'All';position:Position;auction:string[];record:Card[];warnings:string[]}
...
export interface Constraint {seat:Seat;minHcp?:number;maxHcp?:number;lengths?:Partial<Record<Suit,[number,number]>>;expression?:string;partnershipHcp?:[number,number]}
