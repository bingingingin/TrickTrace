...
import {deck,validate,turn,side,suit,rank,legalCards,remainingTricks} from '../core/cards';
...
export function sampleDeals(p:Position,amount:number,seed:number,constraints:Constraint[],progress?:(n:number)=>void){
 ...
 const known=SEATS.filter(s=>p.hands[s]!==null);if(known.length<2)throw Error('实验分析至少需要两家可见手牌');
