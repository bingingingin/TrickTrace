...
import {constrainedDealer} from './constrained-dealer';
...
export function sampleDeals(p:Position,amount:number,seed:number,constraints:Constraint[],progress?:(n:number)=>void,minimumKnown=2){
 ...
 const played=Object.fromEntries(SEATS.map(s=>[s,[...p.current.filter(x=>x.seat===s).map(x=>x.card),...p.history.flatMap(t=>t.cards.filter(x=>x.seat===s).map(x=>x.card))]])) as Record<Seat,Card[]>;
 const sizes=unknown.map(s=>cardsPerHand-p.current.filter(c=>c.seat===s).length);
 const hasBounds=unknown.some(s=>voids[s].size||constraints.some(c=>c.seat===s&&Object.keys(c.lengths??{}).length));
 const deal=hasBounds?constrainedDealer(pool,unknown,sizes,constraints,played,voids,rng):()=>{
  ...
  let offset=0;return sizes.map(n=>{const h=shuffled.slice(offset,offset+n);offset+=n;return h;});
 };
 const limit=Math.max(10000,amount*1000);
 while(samples.length<amount&&attempts<limit){attempts++;
  const hands={...p.hands},dealt=deal();unknown.forEach((s,i)=>hands[s]=dealt[i]);
  const q={...p,hands};
  ...
  samples.push(structuredClone(q));progress?.(samples.length);
 }
...
export function validateOpeningLead(p:Position,amount:number){
