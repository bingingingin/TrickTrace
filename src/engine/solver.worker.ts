...
import {analyseOpeningLeads,analyseSamples,sampleDeals,validateOpeningLead,openingLeadPosition} from './sampling';
...
self.onmessage=async({data})=>{const {id,method,args}=data;try{
 if(method==='leadDeals'){const p=openingLeadPosition(args[0]);validateOpeningLead(p,args[1]);const result=sampleDeals(p,args[1],args[2],args[3],undefined,1);self.postMessage({id,result});return;}
 await init();const progress=(n:number)=>self.postMessage({id,progress:n});let result;
