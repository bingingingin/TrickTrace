...
import {analyseOpeningLeads,analyseSamples,sampleDeals,validateOpeningLead} from './sampling';
...
self.onmessage=async({data})=>{const {id,method,args}=data;try{
 if(method==='leadDeals'){validateOpeningLead(args[0],args[1]);const result=sampleDeals(args[0],args[1],args[2],args[3],undefined,1);self.postMessage({id,result});return;}
